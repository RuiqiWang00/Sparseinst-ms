...
class InstanceContextEncoder(nn.Cell):
	def __init__(self,cfg,input_shape):
		...
		for in_channel in reversed(self.in_channels):
			lateral_conv = Conv2d(in_channel, self.num_channels, 1,has_bias=True)
			output_conv = Conv2d(self.num_channels, self.num_channels, 3,has_bias=True)
	...
	def construct(self, features): #feature:dict
		...
		for feature, lat_conv, output_conv in zip(features[1:], self.fpn_laterals[1:], self.fpn_outputs[1:]):
			...
			top_down_features = ops.ResizeNearestNeighbor(size=(h,w))(prev_features)###
